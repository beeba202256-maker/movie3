...
def index():
    ...
    top_rated_movies = sorted(MOVIES, key=lambda x: x['rating'], reverse=True)[:10]
...
def movie_detail(movie_id):
    ...
    related_movies = [m for m in MOVIES if m['id'] != movie_id]
    return render_template('detail.html', movie=movie, movies=MOVIES, related_movies=related_movies)
